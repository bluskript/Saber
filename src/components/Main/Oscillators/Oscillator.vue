...
const modifiers = reactive<IModifier[]>([
  ...
  { name: 'detune', shortName: 'DET', value: detunePercent, min: 0, max: 1, step: 0.01 },
...
<template>
  <div class="bg-harmonydark-700 rounded-8px flex overflow-hidden" :class="{ disabled }">
    ...
    <div class="banner p-2 bg-harmonydark-400 flex flex-col justify-between">
      <div :class="{disabled}" class="soloButton cursor-pointer rounded-full w-5 h-5 bg-primary-300" @click="disabled = !disabled"></div>
